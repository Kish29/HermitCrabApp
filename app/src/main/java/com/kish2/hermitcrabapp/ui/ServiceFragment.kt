...
class ServiceFragment : BaseFragment(R.layout.hermitcrab_fragment_service), MavericksView, TabFragmentApi {
	...
	override fun getTabView(layoutInflater: LayoutInflater): View {
		...
		tabView.findViewById<ImageView>(R.id.ivTabIcon).setImageDrawable(
			AppVectorImg.viServiceIconDft?.let { dft ->
				AppVectorImg.viServiceIconAcv?.let { acv ->
		...
		tabView.findViewById<TextView>(R.id.tvNavName).apply {
			...
			text = R.string.hermitcrab_nav_service_title.getString()
		}
		return tabView
	}
