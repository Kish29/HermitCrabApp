...
///////////////////////////////////////////////////////////////////////////
// 视频等比例放大,直至一边铺满VideoView(或屏幕)的某一边,另一边超出View的另一边,再移动到View的正中央,这样长边两边会被裁剪掉同样大小的区域,视频看起来不会变形,
// 也即是:先把视频区(实际的大小显示区)与View(定义的大小)区的两个中心点重合, 然后等比例放大或缩小视频区,直至一条边与View的一条边相等,另一条边超过View的另一条边
// 这时再裁剪掉超出的边, 使视频区与View区大小一样. 这样在不同尺寸的手机上,视频看起来不会变形,只是水平或竖直方向的两端被裁剪了一些.
// 以上需求用系统的VideoView是无法达到的,所以要自定义VideoView.
// 最终我们扩展TextureView来自定义一个TextureVideoView, 并应用Matrix进行缩放, 也即是要求出变换的Matrix
 ...
 * </ol>
 */
public class CustomTextureVideoView extends TextureView
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(CustomTextureVideoView.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(CustomTextureVideoView.class.getName());
    }
    ...
    private void transformVideo(int videoWidth, int videoHeight) {
        int vWidth = getResizedWidth();
        int vHeight = getResizedHeight();
        if (vHeight == 0 || vWidth == 0) {
            Log.d(TAG, "transformVideo, getResizedHeight=" + vHeight + "," + "getResizedWidth=" + vWidth);
            return;
        }
        float sx = (float) vWidth / (float) videoWidth;
        float sy = (float) vHeight / (float) videoHeight;
        ...
        matrix.preTranslate((vWidth - videoWidth) / 2, (vHeight - videoHeight) / 2);

        //第1步:因为默认视频是fitXY的形式显示的,所以首先要缩放还原回来.
        matrix.preScale(videoWidth / (float) vWidth, videoHeight / (float) vHeight);

        //第3步,等比例放大或缩小,直到视频区的一边超过View一边, 另一边与View的另一边相等. 因为超过的部分超出了View的范围,所以是不会显示的,相当于裁剪了.
        matrix.postScale(maxScale, maxScale, vWidth / 2, vHeight / 2);//后两个参数坐标是以整个View的坐标系以参考的
